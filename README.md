...
Marketing site for Geekin's Canadian brand (`geekin.ca`) — a React + Vite
single-page app (the "Holographic Ops HUD" design, cyan accent) plus a small
Express + Nodemailer API that sends the contact form over SMTP.

This is the Canada-only counterpart of [geekin-us](https://github.com/donbaba1/geekin-us) —
same codebase, forked apart so each region can be deployed, edited, and
color-themed independently. There's no runtime region switch here; cyan is
...
serves static files from, e.g. `/var/www/geekin-ca`.
...
```ini
# /etc/systemd/system/geekin-ca-api.service
[Unit]
Description=Geekin CA contact-form API
After=network.target

[Service]
WorkingDirectory=/opt/geekin-ca/server
ExecStart=/usr/bin/node index.js
EnvironmentFile=/opt/geekin-ca/server/.env
Restart=on-failure
User=www-data
...
cp .env.example .env                  # fill in real SMTP credentials
sudo systemctl enable --now geekin-ca-api
```

(`pm2 start index.js --name geekin-ca-api` works just as well if that's what
you'd rather use.)
